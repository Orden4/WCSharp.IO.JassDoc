...
namespace WCSharp.IO.JassDoc.Parsing
{
	public static class JassDocParser
	{
		...
		public static async Task<JassApi> ParseDocAsync(params JassDocTarget[] targets)
		{
			...
			{
				string content;
				var sourceFileName = target.FileName ?? target.Uri.Segments[^1];

				if (target.Uri.IsFile)
				{
					content = await File.ReadAllTextAsync(target.Uri.LocalPath);
				}
				else
				{
					using var httpClient = new HttpClient();
					using var response = await httpClient.GetAsync(target.Uri);
					if (response.StatusCode != HttpStatusCode.OK)
						throw new InvalidOperationException($"Unable to retrieve jassdoc document at {target.Uri} (status code: {response.StatusCode})");
		...
		private static JassApi ParseDoc(string input, string sourceFileName)
		{
